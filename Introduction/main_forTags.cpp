...
struct Base {
    Health hp{};
    // Damage dmg;
    Data<int, Tags<extra_token>> type{};
...
template <>
struct UserDefaultValue<Damage, Tags<extra_token>> {
...
template <>
struct std::formatter<Data<int, Tags<extra_token>>> : std::formatter<std::string> {
    auto format(const Data<int, Tags<extra_token>>& t, [[maybe_unused]] std::format_context& ctx) const {
        std::ostringstream o;
        o << t;
...
auto for_each_taged_type = for_each_impl<[]<size_t N, typename T>(T&& t) {
    if constexpr (requires { getTaged<N, Tags<extra_token>>(std::declval<T>()); }) {
        return getTaged<N, Tags<extra_token>>(std::forward<T>(t));
    } else {
...
int main() {
    auto create_entity =                                     //
        ...
        | With::user_property<Health, Tags<extra_token>>     //
        | With::user_property<int, Tags<extra_token>>        //* similiar with Base::type
        | With::user_property<Data<int, Tags<extra_token>>>  //* skipped for Base
        ;

    auto entity = create_entity(
        unordered,
        forTags<>(Health{100}),
        forTags<Tags<extra_token>>(Health{150}),
        forTags<>(Damage{5}),
        forTags<Tags<extra_token>>(Damage{150}),
        CureHealth{15},  //* can mix with normal types
        forTags<Tags<extra_token>>(25),
        // Data<int, extra_token>{15}, //! duplication of types if Base - Data is used as way to specify type in C-tor
        forTags<>(Data<int, Tags<extra_token>>{15}));  //* walkaround - Data<Data<int, extra_token>>
    ...
    auto tied_data = std::tie(
        getTaged<0, Tags<extra_token>>(entity),
        getTaged<1, Tags<extra_token>>(entity),
        getTaged<2, Tags<extra_token>>(entity));
    std::println();
